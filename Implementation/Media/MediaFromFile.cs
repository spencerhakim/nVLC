...
namespace Implementation.Media
{
    internal class MediaFromFile : BasicMedia, IMediaFromFile
    {
        ...
        public override string Input
        {
            ...
            {
                m_path = value;
                m_hMedia = NativeMethods.libvlc_media_new_path(m_hMediaLib, m_path.ToUtf8());
            }
        }

        public string GetMetaData(MetaDataType dataType)
        {
            IntPtr pData = NativeMethods.libvlc_media_get_meta(m_hMedia, (libvlc_meta_t)dataType);
            return Marshal.PtrToStringAnsi(pData);
        }

        public void SetMetaData(MetaDataType dataType, string argument)
        {
            NativeMethods.libvlc_media_set_meta(m_hMedia, (libvlc_meta_t)dataType, argument.ToUtf8());
        }

        public void SaveMetaData()
        {
            NativeMethods.libvlc_media_save_meta(m_hMedia);
        }
        ...
        public long Duration
        {
            get
            {
                return NativeMethods.libvlc_media_get_duration(m_hMedia);
            }
        }
        ...
        public MediaTrackInfo[] TracksInfo
        {
            get
            {
                ...
                int num = NativeMethods.libvlc_media_get_tracks_info(m_hMedia, out pTr);
